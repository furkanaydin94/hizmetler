...
function mainApp() {
    ...
    if (!locationFilter && localStorage.getItem('viewer_location')) {
        ...
        console.log('📍 Location localStorage\'dan alındı:', locationFilter);
    }
    if (!reportMonthParam && localStorage.getItem('viewer_reportMonth')) {
        ...
        console.log('📅 ReportMonth localStorage\'dan alındı:', reportMonthParam);
    }
    if (!turlerParam && localStorage.getItem('viewer_turler')) {
        ...
        console.log('🏷️ Turler localStorage\'dan alındı:', turlerParam);
    }
    if (!autoPrint && localStorage.getItem('viewer_autoprint')) {
        ...
        console.log('🖨️ AutoPrint localStorage\'dan alındı:', autoPrint);
    }
    ...
    // Her zaman serviceConfig'i oku çünkü en kapsamlı filtre bilgisini içerir
    try {
        const savedConfig = localStorage.getItem('serviceConfig');
        console.log('🔍 serviceConfig kontrolü:', savedConfig ? 'MEVCUT' : 'YOK');

        if (savedConfig) {
            const config = JSON.parse(savedConfig);
            console.log('📋 serviceConfig yüklendi:', {
            ...
            // Aktif türleri çıkar (önce türler - hizmet filtrelemede kullanılacak)
            if (config.turler) {
                selectedTurler = Object.keys(config.turler).filter(t => config.turler[t] !== false);
                console.log('✅ Aktif türler:', selectedTurler.length, selectedTurler);
            }

            // Aktif hizmetleri çıkar
            if (config.services) {
                // hizmetTurler varsa daha detaylı filtreleme yap
                ...
                console.log('✅ Aktif hizmetler:', selectedServices.length);
            }
        }
    } catch (e) {
        console.warn('serviceConfig yüklenemedi:', e);
    }
    ...
    // Rapor tarihine göre veri filtrele (veriAralığı tipine göre)
    // - BGG/AYLIK: Sadece seçilen yılın seçilen ayı
    // - KÜMÜLATİF: Geçmiş yılların Aralık + seçilen yılın seçilen ayı
    const filterByReportMonth = (data) => {
        ...
        return data.filter(r => {
            ...
            // Seçilen yıldan sonraki aylar: Dahil etme
            if (dataYear === reportYear && dataMonth > reportMonth) return false;

            // BGG veya AYLIK: Sadece seçilen yılın seçilen ayı
            ...
            // Diğer tipler için: seçilen yılın seçilen ayı
            return dataYear === reportYear && dataMonth === reportMonth;
        });
    };
    ...
    async function loadData() {
        try {
            // 1. Önce IndexedDB'den kontrol et (5MB sınırı yok, tercih edilen yöntem)
            ...
            if (indexedDBData && indexedDBData.length > 0) {
                // IndexedDB'den geçici veri varsa onu kullan
                console.log('✅ IndexedDB\'den geçici veri yükleniyor:', indexedDBData.length, 'kayıt');
